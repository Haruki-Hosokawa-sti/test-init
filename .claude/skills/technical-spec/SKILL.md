...
## Docker Compose ポート競合管理ルール
- `docker compose up` 前にポート競合チェックを実施すること
- 同一docker-compose.yml管理下のコンテナのみ自動停止を許可
- 他プロジェクトのコンテナは手動対応を促すこと

## WorkTree 運用ルール
- 新規要件は `feature/<タスク概要>` ブランチで作業すること
- mainブランチへの直接pushは禁止
- 作業完了時は `gh pr create` でPRを作成すること
